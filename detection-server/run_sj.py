...
class Daemon:
    ...
    def start(self):
        ...
        while True:
            ...
            for cw_id, cw in self.crosswalks.items():
                if cw['matched'] is False: continue
                ...
                sql = "UPDATE `objhistory` set content='%s', updated_at='%s' where intersection_id='%d' and crosswalk_id='%d';"%(pymysql.escape_string(str(obj_infos).replace("\'", "\"")), datetime.now(),cw['intersection_id'], cw['id'])
            ...
            for cw_id, cw in self.crosswalks.items():
                color = (0, 0 , 255)
                if cw['matched'] is True:
                    color = (0, 255, 0)
                xy = np.array(cw['rects'], dtype=np.int32) * self.imageScale
                center = (cw['center'][0] * self.imageScale, cw['center'][1] * self.imageScale)
    ...
    def redetected_crosswalk(self):
        ...
        '''
        sql = "SELECT * FROM `crosswalk` WHERE cctv_id=0;"
        cursor = self.dbConn.cursor(pymysql.cursors.DictCursor)
        cursor.execute(sql)
