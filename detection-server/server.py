...
class ServerSocket:
    ...
    def socketOpen(self):
        ...
        self.sock.listen(1)
        logger.info(
            u'Server Listening at [ TCP_IP: ' + self.TCP_IP + ', TCP_PORT: ' + str(self.TCP_PORT) + ' ] is open')
    ...
    def receiveImages(self):
        ...
        try:
            while True:
                ...
                # if cnt < 100: print('send time: ' + stime.decode('utf-8'))
                now = time.localtime()
                # if cnt < 100:
                ...
                if self.recentImage is not None:
                    if len(self.recentImage) is 0:
                        self.recentImage.append(decimg)
                    else:
                        self.recentImage[0] = copy.copy(decimg)

                # cv2.imshow("image", decimg)
                                #cv2.waitKey(1)
                if (cnt == 60 * 10 * 10):
                    cnt = 0
                    convertThread = threading.Thread(target=self.convertImage,
                                                     args=(6000, startTime))
                    convertThread.start()
                    self.folder_num = (self.folder_num + 1) % 2
        except Exception as e:
            print(e)
            self.convertImage(cnt, startTime)
            self.socketClose()
            self.serverOpenThread = threading.Thread(target=self.socketOpen)
            self.serverOpenThread.start()
    ...
    def convertImage(self, count, now):
        img_array = []
        cnt = 0
        for filename in glob.glob('./' + str(self.TCP_PORT) + '_images' + str(self.folder_num) + '/*.jpg'):
            if (cnt == count):
                break
            cnt = cnt + 1
        ...
        name = 'project('+ file_date + '.' + file_time + ').mp4'
        file_path = '../node-server/public/videos/' + name
    ...
    def dbConnect(self):
        ...
        logger.debug(u'Video Database is connected')
    ...
    def insertDB(self, file_name, file_date):
        ...
        print(file_path)
        sql = """insert into video (name, date, frame, path) values ("%s", "%s", "%d", "%s")""" % (
            file_name, file_date, 10, file_path)
        try:
    ...
    def startReceiveRecentImage(self, recentImg):
        self.recentImage = recentImg
